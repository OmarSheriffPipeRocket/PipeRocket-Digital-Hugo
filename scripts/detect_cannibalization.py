...
Two independent layers, then cross-referenced (the design's payoff):
  ...
  Layer A — DECLARATIVE (no GSC): collisions between pages' normalized
            primary / canonical_for / secondary in data/keyword_map.yml.
...
REPORTS ONLY — never mutates keyword_map.yml. Writes:
...
from build_keyword_map import normalize  # shared tokenizer (lowercase+stopword+singularize)

ROOT = Path(__file__).resolve().parent.parent
MAP_FILE = ROOT / "data" / "keyword_map.yml"
...
# ---------- parse keyword_map.yml (hand-rolled; no PyYAML in this env) ----------
...
def recommend(group, page_by_path, example_query):
    ...
    for loser in real[1:]:                 # only the genuine competitors, not noise URLs
        ...
        # URLs not in keyword_map are service/legacy/redirect pages — never auto-recommend
        ...
        if l_page is None or owner_unknown:
            moves.append((loser["path"], "INVESTIGATE",
                          "a competing URL is not in keyword_map (legacy/redirect or service "
...
def build_report():
    ...
    L = [
        ...
        "",
        "Two-layer detection: **declarative** (keyword_map.yml collisions) × "
