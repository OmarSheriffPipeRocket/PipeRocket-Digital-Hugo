...
- **Test one thing at a time:** Change the hook or the visual, not both, or you'll never know which one moved the number. The most common cause of dead tests is changing everything at once.
- **Pick a metric your data can actually support:** Early-stage creative gets judged on the signal you have volume for (click-through, thumb-stop, cost per lead), not on closed-won.
...
Our team has seen creative do most of the work on a LinkedIn program by warming named accounts, with brand searches and direct traffic climbing weeks later. Judge that creative on last-click and you'll kill the ad that was actually working.
...
Judge each creative on the metric its stage can actually produce data for, not on the metric you wish you had. This is where most testing frameworks fall apart in SaaS: they tell you to optimize for conversions when you don't have enough conversions to be significant.
...
| Mid (consideration) | Cost per lead, engagement | Enough leads to compare, not enough deals |
...
Launch enough variants, let each accumulate real impressions, and don't let the algorithm crown a winner off a handful of clicks. On a small B2B audience the temptation is to run two ads and pick one after a day. With that little data you're guessing, not testing.

Our rough operating rules:

- **Launch three to five variants** per ad set on genuinely different angles, not five near-identical headlines
...
## Step 4: Keep a Testing Cadence, Not a One-Off Test
...
### Test audience and funnel stage, not just the creative
